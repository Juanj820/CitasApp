...
class LoginView:
    def __init__(self, root, usuario_controller, on_login_success):
        ...
        # Título del login
        tb.Label(self.frame, text="Iniciar Sesión", font=("Arial", 16, "bold"), bootstyle="primary").pack(pady=(0,20))
        ...
        self.password_entry = tb.Entry(self.frame, show="*", width=30, font=("Arial", 10))
    ...
    def cerrar_aplicacion(self):
        ...
        self.root.destroy()  # Cerrar la aplicación completamente
        self.root.quit()  # Asegurar que la aplicación se cierre
    ...
    # Actualizar la ventana para obtener las dimensiones correctas
        ...
        x = (self.root.winfo_screenwidth() // 2) - (width // 2)
        # Ajustar la posición vertical para que el footer sea visible
        y = (self.root.winfo_screenheight() // 2) - (height // 2) - 50  # Mover 50 píxeles hacia arriba
        # Centrar la ventana
        self.root.geometry(f'{width}x{height}+{x}+{y}')
