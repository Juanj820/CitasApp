...
class CitaView:
    def __init__(self, parent, controller, usuario_id):
        ...
        self.estado_cb = tb.Combobox(busq_frame, values=["", *ESTADOS], textvariable=self.busqueda_estado, width=14, state="readonly")
    ...
    def eliminar_cita(self):
        ...
        if not seleccionado:
            ...
            return
        valores = self.tabla.item(seleccionado[0], "values")
        if messagebox.askyesno("Confirmar", f"¿Eliminar la cita de {valores[4]} con {valores[2]}?"):
            try:
    ...
    def formulario_cita(self, valores=None):
        ...
        tb.Label(form_frame, text="Por favor ingrese la información de la cita",
                font=("Arial", 11)).grid(row=0, column=0, columnspan=2, pady=(0, 15))

        # Crear diccionarios para mapear id a texto
        ...
        paciente_cb = tb.Combobox(form_frame, values=lista_pacientes_nombres, state="readonly", width=28, textvariable=id_paciente)
        ...
        tb.Label(form_frame, text="(AAAA-MM-DD)").grid(row=3, column=2, sticky="w", pady=5, padx=5)
        ...
        def on_paciente_select(event):
                    ...
                    for pid, texto in pacientes_dict.items():
                        if texto == nombre:
                            id_paciente.set(pid)
                            break
        ...
        def validar_hora(hora_str):
            try:
                datetime.datetime.strptime(hora_str, "%H:%M:%S")
                return True
            except ValueError:
                return False
        ...
        def guardar():
            ...
            if not validar_fecha(fecha.get()):
                messagebox.showwarning("Aviso", "Formato de fecha inválido. Use YYYY-MM-DD")
                return

            if not validar_hora(hora.get()):
                messagebox.showwarning("Aviso", "Formato de hora inválido. Use HH:MM:SS")
                return
